  ...
  return `
    <div class="min-h-screen bg-surface flex items-center justify-center text-center px-6" style="min-height:100dvh;">
      <div class="max-w-sm">
        ...
        <button onclick="window.location.hash='/dashboard'" class="px-6 py-3 min-h-[44px] rounded-full bg-primary-container text-on-primary-container font-bold text-sm">Go to Dashboard</button>
      </div>
    </div>`;
}
...
async function init() {
  ...
  onAuthStateChange((event, session) => {
    ...
    if (event === 'SIGNED_IN' && session) {
      const now = Date.now();
      if (now - lastSignInHandled < 1500) return;
  ...
  window.addEventListener('error', (event) => {
    ...
    if (event?.error) toast.error('Something went wrong. Please retry.');
  });
