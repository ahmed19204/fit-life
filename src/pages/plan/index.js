/**
 * FitLife AI Plan Generation Result Page
 * Displays the AI-generated nutrition plan with premium UI.
 */
...
export function renderPlan() {
  ...
  const meals = planData.meal_plan || [];

  return `
    <div class="min-h-screen bg-surface text-on-surface pb-24">
      <!-- Header -->
      <header class="px-5 pt-5 pb-4 text-center relative">
        ...
        <div class="relative z-10">
          <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 border border-primary/20 mb-4">
            ...
            <span class="text-xs font-semibold text-primary uppercase tracking-wider">AI-Generated Plan</span>
          </div>
          <h1 class="text-2xl font-bold mb-1">Your Personalized Plan</h1>
          <p class="text-sm text-on-surface-variant">Crafted by AI based on your unique profile</p>
        </div>
      </header>
          ...
          <div class="grid grid-cols-3 gap-4">
            ${[
              ...
              <div class="text-center">
                <p class="text-xl font-bold" style="color: ${m.color};">${m.value}${m.unit}</p>
                <p class="text-[10px] text-on-surface-variant uppercase tracking-wider mt-0.5">${m.label}</p>
              </div>
            `).join('')}
          </div>
        </div>
              ...
              <div class="p-4 rounded-xl border border-outline-variant/10 bg-surface-container-low/30">
                ...
                ${meal.foods?.length > 0 ? `
                  <div class="flex flex-wrap gap-1.5 mt-2">
                    ${meal.foods.map(f => `<span class="px-2 py-0.5 rounded-full bg-surface-container-high/50 text-[10px] text-on-surface-variant">${f}</span>`).join('')}
                  </div>
                ` : ''}
              </div>
            `).join('') : `
